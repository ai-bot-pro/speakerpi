...
- [ ] 天气播报
- [ ] 生日祝福
- [ ] 声控拍照/视频(物体识别，场景适合幼儿园的小朋友；视频的话可以做一个监控，通过实时监控的视频数据包(packet)通过管道的形式发到莫个服务端口，这个端口可以是本地监听的端口，比如用http协议访问这个端口就可以了(用VLC软件可以实现)，虽然是实时的，但是对于pi来说这种比较耗资源；还有种是方式是将视频数据包发送到中转服务端口上，然后通过中转服务将视频报转发给视频直播的云服务接口，中转服务可以做一层过滤发现，简单的处理方式是通过netcat(nc)将数据包转发到中转服务端口上，通过中转服务做进一步处理(定点上传等)，中转服务访问量不大的情况下，可以直接用nc来取数据，不需要考虑事件机制)
- [ ] todolist
- [ ] 笑话段子
- [ ] 订餐/订票
