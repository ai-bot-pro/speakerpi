...
- [ ] 12.进程之间的交互，可以通过信号，共享空间,这里说的本地进程，选用后者实现(采用Queue或者Pipe的方式实现)；其实这个可以延伸到分布式进程之间的通信，进程之间需要异步处理，采用分布式消息中间件Queue(p2p) 和Topic(pubsub) 进行通信,有利于系统模块的解耦；同步可以采用rpc调用的方式。
